      ...
      details: {
        tasks: {
          en: [
            ...
            'Platform monitoring with Application Insights and Log Analytics: per-agent telemetry separation, console and system logs enabled on the Container Apps environment to surface autoscaler failures, 30-day retention, and a Grafana dashboard for visualization',
            'Azure Monitor metric alerts routed to an action group: execution failure on every job, Service Bus dead-letter accumulation, and messages left unconsumed for over 30 minutes to detect a stalled autoscaler. Alerts are generated from a Terraform map so any new job is covered automatically',
            'Monthly Azure budget with alerts at 80% and 100% to catch cost drift before month end, and product analytics on the web application through PostHog',
            'Orchestrated an AI-assisted development workflow: two Claude roles with separate permissions (Claude Cowork read-only for design and planning, Claude Code for feature implementation), several Claude Code agents working in parallel, each isolated in its own git worktree, hooks and skills enforcing the project conventions and git workflow, dedicated subagents for code review, documentation and code exploration, and a CI/CD pipeline validating unit tests and the terraform plan before a final Claude-driven review',
          ],
          fr: [
            ...
            "Supervision de la plateforme avec Application Insights et Log Analytics : télémétrie séparée par agent, journaux console et système de l'environnement Container Apps activés pour rendre visibles les défaillances de l'autoscaler, rétention à 30 jours, et un dashboard Grafana pour la visualisation",
            "Alertes de métriques Azure Monitor routées vers un groupe d'action : échec d'exécution sur chacun des jobs, accumulation en file de rebut Service Bus, et messages non consommés au-delà de 30 minutes pour détecter un autoscaler à l'arrêt. Les alertes sont générées depuis une map Terraform pour que tout nouveau job soit couvert automatiquement",
            "Budget mensuel Azure avec alertes à 80 % et 100 % pour détecter une dérive de coût avant la fin du mois, et analytics produit sur l'application web via PostHog",
            "Mise en place d'un workflow de développement assisté par agents IA : deux rôles Claude aux permissions séparées (Claude Cowork en lecture seule pour la conception et la planification, Claude Code pour l'implémentation des features), plusieurs agents Claude Code travaillant en parallèle, chacun isolé sur son propre worktree git, hooks et skills pour faire respecter les conventions et le workflow git, subagents dédiés à la revue de code, à la documentation et à l'exploration du code, et une CI/CD validant les tests unitaires et le terraform plan avant une review finale déléguée à Claude",
          ],
        },
        projectLink: {
